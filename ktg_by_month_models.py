...
def ktg_by_month_models(maintanance_jobs_df,eo_calendar_fond, year):
  ...
  maintanance_jobs_df_level_upper = pd.merge(maintanance_jobs_df, level_upper, on = 'level_upper', how = 'left')
  # создаем поле-ключ teh-mesto-month-year 
  maintanance_jobs_df_level_upper['teh_mesto_month_year'] = maintanance_jobs_df_level_upper['level_upper'] + '_' + maintanance_jobs_df_level_upper['month_year']

  month_year_downtime = maintanance_jobs_df_level_upper.groupby(['teh_mesto_month_year','level_upper','Название технического места', 'month_year'], as_index=False)['dowtime_plan, hours'].sum()
  ...
  level_upper_list = maintanance_jobs_df_level_upper['level_upper'].unique()
